...
impl ViewEventPostBoundary for ViewEventPostInteractor {
    async fn apply(
        ...
        let ::core::option::Option::Some(post) = ::std::sync::Arc::clone(&self.post_repository).get_by_id(post_id).await? else {
        ...
        let response = ViewEventPostOkResponse::builder()
            ...
            .created_at(::std::sync::Arc::clone(&self.timestamp_codec).format(::std::sync::Arc::clone(&self.uuid_generator).get_timestamp(post.id).await?).await?)
            .title(post.title)
            .content(post.content)
            .reactions(::std::sync::Arc::clone(&self.reaction_repository).view_by_post_id(post.id).await?.into_stream()
                .then(|reaction| async { ::std::sync::Arc::clone(&self.user_repository).get_by_id(reaction.author_id).await
                    .map(|author| (reaction, author)) })
                .filter_map(|fallible| async { fallible.ok() })
                .then(|(reaction, author)| async {
            ...
            .comments(::std::sync::Arc::clone(&self.comment_repository).view_by_post_id(post.id).await?.into_stream()
                .then(|comment| async { ::std::sync::Arc::clone(&self.user_repository).get_by_id(comment.author_id).await
                    .map(|author| (comment, author)) })
                .filter_map(|fallible| async { fallible.ok() })
                .then(|(comment, author)| async {
                ...
                .filter_map(|fallible| async { fallible.ok() })
                .collect::<::std::vec::Vec<_>>().await)
            .maybe_author(::std::sync::Arc::clone(&self.user_repository).get_by_id(post.author_id).await?.map_async(|author| async {
                ViewEventPostUser::build_from(author)
                    .with_uuid_codec(::std::sync::Arc::clone(&self.uuid_codec))
                    .try_build().await
            }).await.transpose()?)
            .is_reacted_by_actor(::std::sync::Arc::clone(&self.reaction_repository).contains_post_and_user_id(post.id, actor_id).await?)
            .build();
