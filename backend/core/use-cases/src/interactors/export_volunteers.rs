...
impl ExportVolunteersBoundary for ExportVolunteersInteractor {
    async fn apply(self: ::std::sync::Arc<Self>, request: Request) -> ::axiom::result::Fallible<Response> {
        ...
        let bytes = match request.format {
            ExportFormat::Csv => ::std::sync::Arc::clone(&self.user_exporter).export_volunteers_as_csv().await?,
            ExportFormat::Json => ::std::sync::Arc::clone(&self.user_exporter).export_volunteers_as_json().await?,
        };
