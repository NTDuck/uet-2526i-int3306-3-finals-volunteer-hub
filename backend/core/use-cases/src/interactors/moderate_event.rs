    ...
    async fn apply(
        ...
        let actor_id = match ::std::sync::Arc::clone(&self.auth_token_generator).get_payload(request.token).await? {
            ::core::option::Option::None =>
                return ::axiom::err!(ModerateEvent @ AuthenticationTokenInvalid),
        ...
        let ::core::option::Option::Some(mut event) = ::std::sync::Arc::clone(&self.event_repository).get_by_id(event_id).await? else {
        ...
        match request.event_status {
            ModerateEventNewEventStatus::Approved => {
                if !::core::matches!(event_status, ::domain::EventStatus::Created { .. } | ::domain::EventStatus::Updated { .. } | ::domain::EventStatus::Approved { .. }) {
