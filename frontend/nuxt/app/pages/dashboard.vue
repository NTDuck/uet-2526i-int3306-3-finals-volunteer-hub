...
type ViewEventRecommendationRecommendationType = 'recently-published' | 'recently-posted' | 'trending'
...
const approvedRelevantEvents = computed(() => relevantEvents.value.filter((e) => e.status === 'approved'))

const approvedTrendingEvents = computed(() => trendingEvents.value.filter((e) => e.status === 'approved'))

const hasAnyEvents = computed(() => approvedRelevantEvents.value.length > 0 || approvedTrendingEvents.value.length > 0)
...
const fetchRecommendations = async () => {
  ...
  try {
    const [relevant, trending] = await Promise.all([fetchForType('recently-posted', 4), fetchForType('trending', 4)])
      ...
      <section>
        <header class="text-center">
          ...
          <p class="mt-2 text-[1rem] text-gray-600">
            See events that matter to you and discover<br />where the community is most active right now.
          </p>
            ...
            <div v-else class="space-y-3">
              ...
              >
                <header class="flex items-center justify-between gap-2">
                  ...
                  <span class="px-2 py-0.5 rounded-full text-[1rem] font-medium" :class="statusClasses(event.status)">
        ...
        <div v-if="!loading && !loadError && !hasAnyEvents" class="text-center text-xs text-gray-500">
