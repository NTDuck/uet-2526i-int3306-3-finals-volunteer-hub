...
const onSubmit = async () => {
  ...
  try {
    ...
    showErrorMessage.value = true
    errorMessage.value = err.data.err.message ?? 'Unexpected error during registration'
  }
}
</script>
    ...
    <header class="h-16 bg-[#256EB1] text-white shadow-md">
      <div class="h-full px-6 flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <router-link to="/register" class="text-sm text-white">
            <span class="text-xl font-medium">VolunteerHub</span>
          </router-link>
        </div>
        <router-link to="/login" class="text-sm text-white">
          <button
            type="button"
      ...
      <section class="bg-white p-8 rounded-xl shadow-lg">
        ...
        <form class="space-y-6" @submit.prevent="onSubmit">
          ...
          <div class="flex justify-between items-center">
            <router-link to="/login" class="text-[1.1rem] text-blue-600 hover:underline"
