...
import type { RemoveEventPostRequest } from 'volunteerhub'
...
export default defineEventHandler(async (event) => {
  ...
  const request: RemoveEventPostRequest = {
    token,
    postId,
  }

  try {
    const res = await app.removePost(request)
    ...
    return {
      error: 'InternalError',
      message: 'Unexpected error during RemovePost',
      data: ''
    }
  }
})