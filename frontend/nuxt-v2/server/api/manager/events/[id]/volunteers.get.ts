...
import type { ViewEventVolunteersRequest } from 'volunteerhub'
...
export default defineEventHandler(async (event) => {
  ...
  const request: ViewEventVolunteersRequest = {
    token,
    eventId,
  }

  try {
    const res = await app.viewEventVolunteers(request)
    ...
    return {
      error: 'InternalError',
      message: 'Unexpected error during ViewEventVolunteers',
      data: ''
    }
  }
})
