...
import type { RemoveEventRequest } from 'volunteerhub'
...
export default defineEventHandler(async (event) => {
  ...
  const request: RemoveEventRequest = {
    token,
    eventId,
  }

  try {
    const res = await app.removeEvent(request)
    ...
    return {
      error: 'InternalError',
      message: 'Unexpected error during RemoveEvent',
      data: ''
    }
  }
})
