...
import type { CreateEventPostRequest } from 'volunteerhub'
...
export default defineEventHandler(async (event) => {
  ...
  const request: CreateEventPostRequest = {
    token,
    eventId,
  ...
  try {
    const res = await app.createPost(request)
    ...
    return {
      error: 'InternalError',
      message: 'Unexpected error during CreatePost',
      data: ''
    }
  }
})
