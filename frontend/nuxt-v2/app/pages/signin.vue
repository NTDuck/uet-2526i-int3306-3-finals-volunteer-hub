    ...
    <header class="h-16 bg-[#256EB1] text-white shadow-md">
      <div class="h-full px-6 flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <NuxtLink to="/signin" class="text-sm text-white">
            <span class="text-xl font-medium">VolunteerHub</span>
          </NuxtLink>
        </div>
        <NuxtLink to="/signup" class="text-sm text-white">
          <button
            type="button"
      ...
      <section class="bg-white p-8 rounded-xl shadow-lg">
        ...
        <form class="space-y-6" @submit.prevent="onSubmit">
          ...
          <div class="flex justify-between items-center">
            <label
              class="inline-flex items-center space-x-2 text-[1.2rem] text-gray-700 cursor-pointer"
            >
              <input
                v-model="rememberMe"
                type="checkbox"
                class="form-checkbox w-4 h-4 rounded-md text-blue-600"
              />
